...
public class Rat0 extends Robot {
  ...
  public void run() {
    ...
    while (step(timeStep) != -1) {
      ...
      //Distance from the left wall to follow is OK
      boolean left_wall = distance[2] > 310;
      
      //Detected an obstacle in front with one of the 2 back sensors
      boolean obstacle_front = distance[4] > 400 || distance[3] > 400;
      
      //The right hand side back sensor (left front on the direction of backwards movement) detected something
      //This means the robot is doing a 180 left turn and needs to adjust its angle parallel to the wall
      boolean left_corner = distance[3] > 200;
      ...
      if(obstacle_front){
        ...
        if(left_wall){
          ...
          //I lost the wall on the left, let me turn left to find the next one closest
          //System.out.println("Lost the left wall...");
          leftSpeed = maxSpeed;
